      ...
      subcommand
        ...
        .addIntegerOption((option) =>
          option
            .setName("input")
            .setDescription(
              "Number of any messages to try prune, else prune 10, cannot exceed 50"
            )
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addIntegerOption((option) =>
          option
            .setName("input")
            .setDescription(
              "Number of bot messages to try prune, else prune 10, cannot exceed 50"
            )
        )
    ),
    ...
    await interaction.channel.messages
      ...
      .then((b) => {
        ...
        if (interaction.options._hoistedOptions[0]) {
          const check =
            interaction.options._hoistedOptions[0].value > 50 ? true : false;
          const limit = b.splice(
            0,
            check ? 50 : interaction.options._hoistedOptions[0].value
          );
          interaction.channel.bulkDelete(limit, true);
          check
            ? interaction.reply({
                content:
                  interaction.options._subcommand === "bot"
